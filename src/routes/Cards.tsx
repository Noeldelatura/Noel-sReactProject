...
const Cards = () => {
    ...
    return (
        <div className="flex flex-col">
            <div className=" bg-pink-100 m-2 dark:bg-pink-200 border rounded-md search-place pl-6">
                <input
                    type="text"
            ...
            <div className="flex flex-wrap justify-center items-center gap-5 mt-2 dark:text-black">
                {loading && <Spinners />}
                {error && <div>{error}</div>}
                {filteredCards.map((c) => (
                    <div key={c._id} className="flex flex-col justify-center items-center p-5 rounded-md text-center bg-pink-100 m-2 dark:bg-pink-200 relative drop-shadow-md cards-style">
                        <Link to={`/cards/${c._id}`} className="flex flex-col items-center ">
                            ...
                            />
                            <div className="m-1 ">
                                <h2 style={{ color: "rgb(123, 44, 57)" }} className="text-l px-1 py-0.5 font-bold">{c.title}</h2>
                        ...
                        {isAdmin &&
                            <button
                                className={`absolute bottom-10 right-2  mr-2 p-1 rounded-md text-sm bg-pink-100 dark:bg-pink-200 font-medium ${'text-black dark:text-white'}`}
                        ...
                        <button
                            className={`absolute bottom-2 right-2  mr-2 p-1 rounded-md text-sm bg-pink-300 dark:bg-pink-700 font-medium ${favorites.includes(c._id) ? 'text-pink-900 dark:text-pink-200' : 'text-pink-900 dark:text-pink-100'}`}
                        ...
                        <div className={`absolute bottom-2 left-2  mr-2 p-1 px-4  rounded-md text-sm bg-gpink-200 dark:bg-pink-300 ${likedCards.includes(c._id) ? 'text-pink-900 dark:text-pink-200' : 'text-pink-100 dark:text-pink-300'}`}>
